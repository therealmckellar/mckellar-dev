    ...
    >
      <div>
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          <h3 className={`font-display font-bold text-base leading-snug text-on-surface ${accentText}`}>
            {title}
          </h3>
        ...
        {/* Description */}
        <p className="text-[13px] text-on-surface-muted leading-relaxed mb-4">
          {description}
        </p>
        ...
        {useCases.length > 0 && (
          <div className="mb-4 flex flex-col gap-2">
            <p className="text-[10px] font-mono font-semibold uppercase tracking-wider text-on-surface-subtle">
              Key Use Cases
            </p>
            <ul className="flex flex-col gap-1.5">
              {useCases.map((uc, i) => (
                <li key={i} className="flex items-start text-xs text-on-surface-muted leading-relaxed">
        ...
        <div className="flex flex-col gap-1.5 mb-5">
          {links.map((link) => (
            ...
            >
              <ChevronRight />
              <span className="group-hover:text-on-surface transition-colors duration-150 font-medium">
                {link.label}
              </span>
            </a>
          ))}
      ...
      {ctaLabel && onCtaClick && (
        <button
          onClick={onCtaClick}
          className={`w-full py-2.5 px-4 rounded-lg text-xs font-semibold glass-button flex items-center justify-center gap-2 group-hover:bg-white/10 transition-all active:scale-[0.98] ${
            accent === 'blue' 
